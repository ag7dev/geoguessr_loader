...
// Hier wird die Funktion angepasst, um den benutzerdefinierten Offset zu verwenden.
function getRandomOffset() {
    let customOffset = parseFloat(localStorage.getItem('customOffset'));
    if (isNaN(customOffset)) {
        customOffset = 2.0; // Standardwert
        localStorage.setItem('customOffset', customOffset);
    }
    // Zufälliger Wert zwischen -customOffset und +customOffset
    return (Math.random() * 2 * customOffset) - customOffset;
}

window.addEventListener('load', async function () {

    // Safe Mode als Boolean aus localStorage holen und ggf. initialisieren
    let safeMode = stringToBool(localStorage.getItem('safeMode'));
    if (safeMode === null) {
    ...
    setInterval(() => {
        ...
        if (settingsMenu) {
            ...
            if (childElems.length < 4) {
                settingsMenu.innerHTML += `<label class="game-options_option__xQZVa game-options_editableOption__0hL4c">
                    <img alt="Emoji icon" loading="lazy" width="24" height="24" decoding="async" data-nimg="1" class="game-menu_emoteIcon__t4FxY" src="${safeImg}" style="filter: invert(1);">
                    <div class="game-options_optionLabel__Vk5xN">safe mode</div>
                    <div class="game-options_optionInput__paPBZ">
                        <input type="checkbox" class="toggle_toggle__qfXpL a" ${safeMode ? "checked" : ""}>
                    </div>
                </label>`;
                // Neuer Input für den Random Offset
                settingsMenu.innerHTML += `<label class="game-options_option__xQZVa game-options_editableOption__0hL4c">
                    <div class="game-options_optionLabel__Vk5xN">Random Offset Range</div>
                    <div class="game-options_optionInput__paPBZ">
                        <input type="number" step="0.1" min="0" class="offset_input" value="${localStorage.getItem('customOffset') || 2}">
                    </div>
                </label>`;
            }
            ...
            // Event-Listener für den Offset-Input
            const offsetInput = document.querySelector('.offset_input');
            offsetInput.addEventListener('change', function () {
                localStorage.setItem('customOffset', parseFloat(this.value) || 0);
            });
        }
    }, 50);
});
...
document.addEventListener('keydown', async function (event) {
    ...
    if (event.ctrlKey && event.code === 'Space' && localStorage.getItem('safeMode') == 'false') {
        autoPlace(stringToBool(localStorage.getItem('safeMode')));
    }
...
async function autoPlace(safeMode) {
    ...
    if (element) {
        ...
        if (key) {
            ...
            // Nur im Safe Mode den zufälligen Offset anwenden
            if (safeMode) {
                lat += getRandomOffset();
