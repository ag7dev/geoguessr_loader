...
function convertCoords(lat, long) {
    let latResult = Math.abs(lat);
    let longResult = Math.abs(long);
    let dmsResult = Math.floor(latResult) + "°" + convertToMinutes(latResult % 1) + "'" + convertToSeconds(latResult % 1) + '"' + getLatDirection(lat);
...
window.addEventListener('load', async function () {
    ...
    let autoSubmit = stringToBool(localStorage.getItem('autoSubmit'));
    if (autoSubmit === null) {
        ...
        if (settingsMenu) {
            ...
            if (childElems.length < 4) {
                settingsMenu.innerHTML += `<label class="game-options_option__xQZVa game-options_editableOption__0hL4c">
                    <img alt="Emoji icon" loading="lazy" width="24" height="24" decoding="async" class="game-menu_emoteIcon__t4FxY" src="${safeImg}" style="filter: invert(1);">
                ...
                settingsMenu.innerHTML += `<label class="game-options_option__xQZVa game-options_editableOption__0hL4c">
                    <div class="game-options_optionLabel__Vk5xN">Auto Submit</div>
                    <div class="game-options_optionInput__paPBZ">
                        <input type="checkbox" class="auto_submit_toggle" ${autoSubmit ? "checked" : ""}>
                    </div>
                </label>`;
            }
            this.document.querySelector('.toggle_toggle__qfXpL.a').addEventListener('change', async function () {
                localStorage.setItem('safeMode', this.checked);
            });
            ...
            const autoSubmitToggle = document.querySelector('.auto_submit_toggle');
...
async function autoPlace() {
    const safeMode = stringToBool(localStorage.getItem('safeMode'));
    const autoSubmit = stringToBool(localStorage.getItem('autoSubmit'));
